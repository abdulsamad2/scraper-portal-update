...
export default function InventoryWatcherPage() {
  ...
  return (
    <div className="p-6 max-w-6xl mx-auto space-y-6">
      ...
      {/* The buy-list itself lives under Seat Drops -> Underpriced, next to the
          other "what should I act on right now" view. */}
      <div className="bg-white rounded-xl border border-slate-200 px-5 py-4 flex items-center gap-3 flex-wrap">
        <span className="w-6 h-6 rounded-md bg-emerald-50 flex items-center justify-center">
          <Tag size={12} className="text-emerald-600" />
        </span>
        <p className="text-sm text-slate-600">
          <span className="font-semibold text-slate-700">
            {status.underpriced?.length ?? 0} underpriced listing{(status.underpriced?.length ?? 0) === 1 ? '' : 's'}
          </span>{' '}
          detected — {Math.round((status.underpricedPct ?? 0) * 100)}% or more below the average of the rows behind them.
        </p>
        <a href="/dashboard/drops?tab=underpriced"
           className="ml-auto inline-flex items-center gap-1 px-3 py-1.5 rounded-lg bg-emerald-600 hover:bg-emerald-700 text-white text-xs font-semibold transition-colors">
          Open the buy-list <ExternalLink size={10} />
        </a>
      </div>
