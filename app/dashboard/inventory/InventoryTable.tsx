...
const InventoryTable: React.FC<InventoryTableProps> = ({ data }) => {
  ...
  const columns: TableColumn<InventoryRow>[] = [
    ...
    { name: <CustomHeader title="Cost" description="The cost of the tickets." />, selector: (r) => r.inventory?.cost ?? 0, sortable: true, right: true, width: "80px" },
    { name: <CustomHeader title="List Price" description="The price at which the tickets are listed for sale." />, selector: (r) => r.inventory?.listPrice ?? "-", sortable: true, right: true, width: "100px" },
    ...
    { name: <CustomHeader title="Notes" description="Internal notes about the inventory." />, selector: (r) => r.inventory?.notes ?? "-", width: "200px" },
