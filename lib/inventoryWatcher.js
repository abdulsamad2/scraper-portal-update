 ...
 *
 * Distinct from the undercut alert above, which fires only for resale, only on
 * a listing's first appearance, and only against the section MINIMUM.
 */
const UNDERPRICED_PCT = Number(process.env.UNDERPRICED_PCT ?? 0.35);
// Below this many comparable listings an "average" is just noise.
const UNDERPRICED_MIN_COMPARABLES = Number(process.env.UNDERPRICED_MIN_COMPARABLES ?? 3);
// Keeps the portal's buy-list bounded; highest % below average wins.
const MAX_UNDERPRICED = 60;
/**
 ...
 * inventing them, and it is the bargain we are looking for in the first place.
 *
 * An excluded listing is still evaluated on its own account -- it just stops
 * setting the baseline for its neighbours.
 */
const UNDERPRICED_OUTLIER_MULTIPLE = Number(process.env.UNDERPRICED_OUTLIER_MULTIPLE ?? 3);
