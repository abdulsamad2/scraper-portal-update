 ...
 * should be. Rows are ordered by the rowRank the scraper reads off the row
 * label.
 *
 * A rank only orders rows written the same way. Row 1, row A, row AA and row
 * AAA are all rank 1 and are four different seats, so a section carrying more
 * than one shape is compared shape by shape: the doubled-letter rows are the
 * comparables for a doubled-letter row, and the numbered rows for a numbered
 * one. Merging them would call row AA the equal of row 1 and average a bargain
 * against seats nobody would trade it for.
 *
...
/**
 * The comparables a listing is judged against: its section, narrowed to the
 * rows written the same way. Null when the label is on no scale at all, which
 * is the caller's signal that there is no order to judge it in.
 */
export const comparableKey = (eventId, section, row) => {
  const kind = rowRankKind(row);
  return kind === null ? null : `${sectionKey(eventId, section)}|${kind}`;
};
...
export function findUnderpricedRows(state, dirtySectionKeys) {
  ...
  for (const [key, v] of state.snapshot) {
    ...
    if (!dirtySectionKeys.has(sectionKey(v.eventId, v.section))) continue;
    // Dirty sections are tracked section-wide, as the watcher reports them, but
    // comparables are narrowed to one rank scale: a rank orders rows only
    // against rows labelled the same way.
    const k = comparableKey(v.eventId, v.section, v.row);
    if (k === null) continue;
    const entry = { ...v, key };
